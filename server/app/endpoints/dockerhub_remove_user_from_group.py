...
"""Handler for removing a user from an existing DockerHub group in the Apache org"""
...
@asfquart.APP.route(
    "/api/dockerhub-remove-user-from-group",
    methods=[
        "POST",  # Remove a user from an existing DockerHub group
    ],
)
@asfquart.auth.require
async def process_dockerhub_remove_user_from_group():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as client:
            resp = await client.delete(
                f"{DOCKERHUB_API}/orgs/{org}/groups/{group}/members/{user}",
                headers=headers,
            )
    ...
    await log.slack(
        f"DockerHub user `{user}` has been removed from group `{group}` in the `{config.dockerhub.org}` org, "
        f"as requested by {session.uid}@apache.org."
    )

    return {
        "success": True,
        "message": f"User '{user}' removed from group '{group}'.",
    }
